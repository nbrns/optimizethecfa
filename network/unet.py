"""Minimalistic UNet implementation provided by Hannah Droege, University of Siegen."""
import torch
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, width=64, bilinear=True):
...
class WeakUNet(nn.Module):
    def __init__(self, n_channels, n_classes, width=64, bilinear=True):
        ...
        self.inc = DoubleConv(n_channels, width)
        self.down1 = Down(width, width * 2)
        self.down2 = Down(width * 2, width * 2)
        #self.down3 = Down(width * 4, width * 4)
...
#        x4 = self.down3(x3)  # Out:512
        ...
        #x = self.up1(x5, x4)  # In: 512,512   Out: 256
 #       x = self.up2(x4, x3)  # In: 256,256   Out: 128
        x = self.up3(x3, x2)  # In: 128,128   Out: 64
        x = self.up4(x, x1)  # In: 64,64     Out: 64
